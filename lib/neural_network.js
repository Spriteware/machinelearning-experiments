            ...
            window.requestAnimationFrame(function() {
                ...
                */
                ...
                for (o = 0; o < oel; o++) {
                    ...
                    graph_ctx.lineTo(o, _CANVAS_GRAPH_HEIGHT - output_errors[o] * _CANVAS_GRAPH_HEIGHT);
    
                    // Graphically separate epochs
                    if (epochs < 10 && o > 0 && o % training_size === 0) {
                        graph_ctx.save();
                        graph_ctx.fillStyle = "#d65f42";
                        graph_ctx.fillRect(o, 0, 2 / scaled_width, _CANVAS_GRAPH_HEIGHT);
                        graph_ctx.restore();
                    }

                    // Compute moving averages for a smooth curve
                    if (epochs <= _CANVAS_GRAPH_EPOCHS_TRESHOLD)
                        tmp = Math.sqrt(output_errors[o]) || 0;
                    else
                        tmp = output_errors[o] || 0;
                ...
                graph_ctx.lineTo(o, _CANVAS_GRAPH_HEIGHT);
                graph_ctx.closePath();
                graph_ctx.fill();
                ...
                for (i = 0, l = moving_averages.length; i < l; i++)
                    graph_ctx.lineTo(i, _CANVAS_GRAPH_HEIGHT - (moving_averages[i]) * _CANVAS_GRAPH_HEIGHT);

                graph_ctx.lineTo(oel, _CANVAS_GRAPH_HEIGHT - (moving_averages[i-1]) * _CANVAS_GRAPH_HEIGHT);
                graph_ctx.stroke();
                graph_ctx.restore();
