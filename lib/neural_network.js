...
var Network = function(params) {
    ...
    this.hiddenLayerFunction = "linear"; // activation function for hidden layer
...
Network.prototype.train = function(training_data, epochs, visualise) {
    ...
    var scaled_width, training_size;
    ...
    if (visualise)
    {
        ...
        graph_ctx.scale(scaled_width, 1);
        graph_ctx.globalAlpha = 0.5;

        console.log( "scaled_width", scaled_width, 1/scaled_width );
    }
    ...
    worker.addEventListener("message", function(e) {
        ...
        if (e.data.type === WORKER_TRAINING_PENDING)
        {
            ...
            window.requestAnimationFrame(function() {
                ...
                var output_errors_mean = e.data.output_errors_mean;
                var max_output_error = e.data.output_errors_mean;
                var i, l, g, gel = output_errors.length;
                var tmp, sum = 0, values = [], moving_averages = [], _AVERAGES_SIZE = _CANVAS_GRAPH_WIDTH / scaled_width / 10;

                // Display error curve
                graph_ctx.clearRect(0, 0, _CANVAS_GRAPH_WIDTH / scaled_width, _CANVAS_GRAPH_HEIGHT);
                graph_ctx.beginPath();
                ...
                for (g = 0; g < gel; g++) {
                    ...
                    graph_ctx.lineTo(g, _CANVAS_GRAPH_HEIGHT - output_errors[g] * _CANVAS_GRAPH_HEIGHT);
    
                    // Graphically separate epochs
                    if (epochs < 10 && g > 0 && g % training_size === 0) {
                        graph_ctx.save();
                        graph_ctx.fillStyle = "#d65f42";
                        graph_ctx.fillRect(g, 0, 2 / scaled_width, _CANVAS_GRAPH_HEIGHT);
                        graph_ctx.restore();
                    }

                    // Compute moving averages for a smooth curve
                    tmp = Math.sqrt(output_errors[g]) || 0;
                    // tmp = Math.sqrt(Math.sqrt(output_errors[g])) || 0;
                    values.push(tmp);
                    sum += tmp;

                    if (g >= _AVERAGES_SIZE) {
                        sum -= values.shift(values);
                        moving_averages.push(sum / _AVERAGES_SIZE);
                    }
                }
                ...
                for (i = 0, l = moving_averages.length; i < l; i++)
                    graph_ctx.lineTo(i + _AVERAGES_SIZE / 2, _CANVAS_GRAPH_HEIGHT - (moving_averages[i]) * _CANVAS_GRAPH_HEIGHT);

                graph_ctx.lineTo(gel, _CANVAS_GRAPH_HEIGHT);
                graph_ctx.stroke();
                graph_ctx.restore();
                // End display smoother curves

                // Update output text display
                text_output.innerHTML = "epoch " + (e.data.curr_epoch+1) + "/" + epochs + " | output error mean: " + output_errors_mean.toFixed(5);
            });
        }
