...
Network.prototype.createVisualization = function() {
    ...
    var width = x2 + _MARGIN_X, height = max_y1 + _MARGIN_Y, scale = 1.5;
...
Network.prototype.backpropagate = function(targets) {
    ...
    for (index = this.layersSum[curr_layer-1] - 1; index >= 0; index--)
    {
        ...
        // Update biais
        neuron.biais = neuron.biais + this.lr * neuron.error; // i'm not so sure about this formula ...
    }
...
Network.prototype.train = function(training_data, epochs, vizualisation) {
    ...
    console.info("Training imported data in processing...");

    // var wk = new Worker("worker-training.js"); 
    ...
    for (curr_epoch = 0; curr_epoch < epochs; curr_epoch++)
    {
        for (i = 0; i < training_size; i++)
        {
            ...
            this.feed(training_data[i].inputs);
            this.backpropagate(training_data[i].targets);
            
            global_errors.push( this.globalError );
            global_errors_sum += this.globalError;
            max_global_error = this.globalError > max_global_error ? this.globalError : max_global_error;
        }
    }

    var global_errors_mean = global_errors_sum / (epochs * training_size); 

    console.info("Training done. Gone thru all epochs", {epochs: epochs, global_errors_mean: global_errors_mean});
    ...
    if (vizualisation === true)
    {
        ...
        global_errors_mean_output = document.createElement("samp");
        container.appendChild( global_errors_mean_output ); 

        graph_ctx = graph.getContext("2d");
        graph_ctx.scale(_CANVAS_GRAPH_WIDTH / (epochs * training_size), 1);
        graph_ctx.globalAlpha = 0.5;

        var g, gel = global_errors.length, std_deviation;
        
        // Compute standart deviation
        for (g = 0, sum = 0; g < gel; g++)
        ...
        graph_ctx.clearRect(0, 0, _CANVAS_GRAPH_WIDTH, _CANVAS_GRAPH_HEIGHT);
        graph_ctx.beginPath();
        graph_ctx.moveTo(0, _CANVAS_GRAPH_HEIGHT);
        ...
        for (g = 0; g < gel; g++) {
            ...
            if (global_errors[g] > global_errors_mean - std_deviation && global_errors[g] < global_errors_mean + std_deviation)
                graph_ctx.lineTo(g, _CANVAS_GRAPH_HEIGHT - global_errors[g] / global_errors_mean * _CANVAS_GRAPH_HEIGHT * 0.2);
        }

        graph_ctx.lineTo(g, _CANVAS_GRAPH_HEIGHT);
        graph_ctx.closePath();
        graph_ctx.fill();
